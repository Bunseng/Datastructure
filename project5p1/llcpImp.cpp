...
{
   //if empty then list does nothing
   if(head == 0) return;

   // delete all odd nodes at starting
   while(head != 0 && head->data % 2 == 1) {
         Node *temp = head;
         head = head->link;
         delete temp;
      }
   ...
   while(ptr != 0) {
      ...
      //we already have prev pointer available
      Node* toRemove = ptr;
      prev->link = ptr->link;
      delete toRemove;
      // prev will remain as it is
      ptr = prev->link;
